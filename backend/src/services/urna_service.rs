...
impl UrnaService {
    ...
    pub async fn process_vote(&mut self, request: UrnaVoteRequest) -> Result<UrnaVoteResponse> {
        ...
        // Criar comprovante
        let receipt = self.create_vote_receipt(&vote, &blockchain_hash).await?;
    ...
    async fn log_vote_event(
        ...
        self.security_service.log_security_event(
            urna.id,
            crate::services::urna_security::SecurityEventType::SoftwareAnomaly,
            crate::services::urna_security::SecuritySeverity::Low,
    ...
    pub async fn generate_health_report(&self) -> Result<crate::services::urna_monitoring::HealthReport> {
